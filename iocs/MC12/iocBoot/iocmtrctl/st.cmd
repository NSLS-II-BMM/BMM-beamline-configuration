...
epicsEnvSet("IOC_ASYN",      "$(SYS){MC:12}")
epicsEnvSet("IOC_PREFIX",    "$(SYS){IOC:MC12}")
epicsEnvSet("CONTROLLER_IP", "10.6.130.92")
...
# disable limit check for axis 6, SampleZ has no limit wired up
##pmacDisableLimitsCheck("M0", 6, 0)
# disable limit check for axis 7, tyd limit has problem
##pmacDisableLimitsCheck("M0", 7, 0)
...
cd ${TOP}
dbl > ./records.dbl
#system "cp ./records.dbl /cf-update/xf06BMc-ioc1.mcc12.dbl"
